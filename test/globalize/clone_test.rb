...
class CloneTest < Test::Unit::TestCase
  ...
  test 'stores translations from cloned created record' do
    cloned = saved_post.clone
    check_stored_translations(cloned)
  end

  test 'stores translations from cloned found record' do
    check_stored_translations( Post.find(saved_post).clone )
  end

  test 'stores translations from cloned reloaded after creation record' do
